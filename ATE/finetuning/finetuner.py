...
# Load the model
logger.info("Loading the model... SIUMMA")
# #MODEL_ID = "unsloth/Meta-Llama-3.1-8B-Instruct-bnb-4bit" maybe wrong as in notebook is unsloth/Meta-Llama-3.1-8B-Instruct
...
if not hf_token:
    logger.error("No Hugging Face token found. Please set HF_TOKEN environment variable or create hf_token.txt")
    raise ValueError("Hugging Face token is required to access Llama models")
...
logger.info("Training complete. Pushing merged model to Hugging Face Hub...")
try:
    model.push_to_hub_merged(
        #"MatteBelle/llama-3.1-8b-instruct-finetunedevaluator", 
        #"MatteBelle/llama-3.2-1b-instruct-finetunedevaluator", 
        "MatteBelle/gemma-3-4b-it-GGUF-finetunedevaluator",
        tokenizer, 
