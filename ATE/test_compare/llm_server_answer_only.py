...
### MODIFIED VERSION OF LLM_SERVER FOR TESTING PURPOSES
...
# # Code to load the base model STARTS
...
#     cache_dir=HF_HOME
# )
# Code to load the base model ENDS


# CODE TO IMPORT FINETUNED MODEL STARTS
try:
    from unsloth import FastLanguageModel
    
    # Load model and tokenizer using unsloth for your fine-tuned model
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="MatteBelle/llama-3.2-1b-instruct-finetunedevaluator",
        max_seq_length=4096,
        #torch_dtype=torch.float16,
        device_map="cuda",
        cache_dir=HF_HOME
    ...
    print(f"DEBUG (Server): Custom model loaded successfully", flush=True)
    
except Exception as loading_error:
    import traceback
    error_details = traceback.format_exc()
    print(f"ERROR: Custom model loading failed: {str(loading_error)}\n{error_details}", flush=True)
# CODE TO IMPORT FINETUNED MODEL STARTS
...
class InferenceRequest(BaseModel):
    ...
    model_name: str = "meta-llama/Llama-3.1-8B-Instruct"  # Default model
