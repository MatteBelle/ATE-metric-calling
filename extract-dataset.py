...
def load_api_list(script_dir):
    ...
    try:
        ...
        # Fall back to ATE directory if not found in script directory
        fallback_path = os.path.join(script_dir, "ATE", "tool_metadata", "API_list.json")
        try:
...
def load_api_descriptions(script_dir):
    ...
    try:
        ...
        # Fall back to ATE directory if not found in script directory
        fallback_path = os.path.join(script_dir, "ATE", "tool_metadata", "API_descriptions.json")
        try:
            with open(fallback_path, 'r', encoding='utf-8') as f:
                api_descriptions = json.load(f)
...
def main():
    ...
    # Find all JSON files in the script directory and ATE/saved_results directory
    json_files = glob.glob(os.path.join(script_dir, "*.json"))
    saved_results_dir = os.path.join(script_dir, "ATE", "saved_results")
